...
## [v0.56] 2026-05-14 — 대전 구 분류 + 어드민 카드 중복제거 표시 수정

### crawler.py
- 대전일자리: title·sub_text에서 대전 5개 구(동구·중구·서구·유성구·대덕구) 추출 → location에 반영
- 기존 jobs.json 대전 공고 57건 소급 수정 완료

### admin-dongnero.html
- 출처 카드 중복제거 행: `dedup_removed`가 비어있을 때 `raw - final`로 폴백 계산하여 표시
