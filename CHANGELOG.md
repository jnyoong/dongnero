...
## [v1.22] 2026-06-26 — 알림톡 저녁 6시 이후 발송 금지

- `notify.py`: 평일 체크 다음에 KST 18시 이후면 발송 스킵하는 시간 체크 추가 (로컬 크롤링 + GitHub Actions 양쪽에 모두 적용됨)
...
- `admin-dongnero.html`: 공고 탭 무한 스크롤 (Infinite Scroll) 적용
  - 기존: 11,000건 전체를 한 번에 innerHTML 렌더링 → 로딩 수 초 소요
  - 변경: IntersectionObserver로 100건씩 점진적 로드
  - 스크롤 하단 200px 진입 시 다음 100건 자동 추가
  - 하단 상태 표시: "N건 중 M건 표시 — 스크롤하면 더 불러옵니다"

---

## [v1.19] 2026-06-01 — 정보글 신규 발행: 돌봄 시터 가이드

- `posts_data.js`: 신규 글 추가 (id: babysitter-senior-2026)
  - 제목: "아이 돌봄 시터, 50대도 됩니다 — 시급 평균 2만4천원, 지금 공고 2천 건 있습니다"
  - 오늘 크롤링 실데이터(1,990건, 시급 평균 24,200원) 기반
- `posts/babysitter-senior-2026.html`: 정적 페이지 생성
- `sitemap.xml`: 15개 포스트 URL 업데이트

---

## [v1.18] 2026-06-01 — 출처별 폴백 시스템 구현 (jobs_backup.json)

- `crawler.py`: 출처별 마지막 정상 수집 백업 파일(`jobs_backup.json`) 관리
  - 크롤링 완료 후 정상 출처만 백업 갱신 (0건이거나 폴백된 출처는 백업 보존)
  - 이상 감지 조건: 0건 or 전일 100건↑ 출처에서 50% 미만 감소
  - 이상 출처는 백업 공고로 자동 대체, 텔레그램 알림에 🔄 폴백 표시
- `jobs_backup.json`: 현재 jobs.json으로 초기 백업 생성 (11개 출처, 11,673건)

---

## [v1.17] 2026-06-01 — 서울일자리포털 파싱 디버깅 로그 추가 + 우편번호 제거

- `crawler.py`: `scrape_seoul_jobs()` 빈 응답 시 상태코드·크기·앞200자 출력 (Actions 원인 추적용)
- `crawler.py`: 지역 파싱에서 우편번호 `(05798)` 형식 제거 정규식 추가
...
- `jobs.html`: 정보글 선택 방식을 단순 랜덤→셔플 큐 방식으로 변경
  - 전체 정보글을 셔플해 순서대로 소진 → 소진 후 재셔플 반복
  - 모든 글이 동일 노출 빈도 보장, 연속 중복 없음
  - `_promoPostIndex` 제거 → `_promoPostQueue` 큐로 교체

## [v1.14] 2026-05-30 — 출처별 수집 캡 상향

- `crawler.py`: 각 출처 수집 캡 조정
  - 고용24: MAX_PAGES 50→100 (1,000→2,000건)
  - 알바몬: pages 4→15 (80→300건)
  - 시니어로: pageSize 100→200 (~1,000→~2,000건)
  - 서울일자리포털: pages 15→30 (1,500→3,000건)
  - 맘시터: MOMSITTER_PAGES 100→200 (1,000→2,000건)
  - 대전일자리: DAEJEON_PAGES 72→100 (720→1,000건)
  - 부산일자리: BUSAN_API_PAGES 5→10 (500→1,000건)
  - 사람인: SARAMIN_PAGES 5→13 (400→~1,000건)
- `CLAUDE.md`: 출처 테이블 캡 수치 업데이트

## [v1.13] 2026-05-30 — 부산일자리 조기종료 수정

- `crawler.py`: 부산일자리 `stop_if_less=1→0` (내부 필터로 0건 반환 시 조기종료 방지), timeout 30→60초
...
- `crawler.py`: `scrape_daejeon()` 매 페이지 새 세션 생성으로 수정
  - 기존: `_session('daejeon')` 재사용 → JSESSIONID 쿠키로 인해 2페이지부터 빈 응답
  - 수정: `requests.Session()` 새 인스턴스 생성 → 모든 페이지 정상 수집
  - 잡아바·어르신일자리 Sheet API 오류는 경기데이터드림 서버 측 문제(JSON→HTML 반환), 외부 이슈 대기

---

## [v1.09] 2026-05-28 — 신규 공공 소스 4개 제거 (타겟층 부적합)

- `crawler.py`: 나라일터·클린아이·인천일자리·경기공공일자리 제거 (10개 출처 유지)
  - 타겟층(50~60대) 적합 공고 비율이 낮아 수집 의미 없음으로 판단
  - 단계 레이블 [X/14] → [X/10] 복원
- `jobs.html`: 4개 소스 CSS 색상 제거
...
- `GROWTH_STRATEGY.md`: 채널 8~13 신규 추가
  - 채널 8: 네이버 SEO 강화 (롱테일 키워드·블로그 연계)
  - 채널 9: 구독자 지인 공유 유도 (알림톡 공유 문구)
  - 채널 10: 스마트폰 교육 강사 협업 (복지관 강사 연계)
  - 채널 11: 교회·성당 주보 (오프라인 정기 노출)
  - 채널 12: 취업박람회 현장 등록
  - 채널 13: 지역 무료 생활정보지
- 채널 비교표 우선순위 열 추가, 즉시실행/이번주/다음주/1개월 구분
- 우선순위 실행 계획 "즉시 실행" 단계 추가
- 채널 전체 지도 업데이트

## [v1.06] 2026-05-28 — 성장 전략 문서 추가

- `GROWTH_STRATEGY.md`: 유저 풀 확장 전략 전체 정리 (채널별 접근법·팜플렛·실행 계획)

## [v1.05] 2026-05-27 — 프로모 카드 정보글 랜덤 표시 수정

- `jobs.html`: `_promoPostIndex` 순환→랜덤 방식으로 변경, 직전 글 중복 방지 로직 추가, 초기값 0→-1

## [v1.04] 2026-05-27 — 정보글 11번째 추가 (퇴직금 안내)

- `posts_data.js`: 11번째 글 추가 — 퇴직금 계산법·신청법·미지급 대처 (severance-pay-2026)
- `posts/severance-pay-2026.html`: 정적 HTML 생성
...
- 수정 전: DB에 같은 폰번호 중복 행이 있으면 모두에게 발송 → 12명 중복 발송 확인
- 수정 후: 78명에게 1건씩만 발송 (원본 90건 → 중복 12건 제거)

## [v0.96] 2026-05-19 — 크롤링 시간 07:00 → 08:30 KST 전체 변경

- `.github/workflows/crawl.yml` — cron `0 22 * * *` → `30 23 * * *` (UTC 23:30 = KST 08:30)
- `setup_scheduler.ps1` — 트리거 07:00 → 08:30 (관리자 권한으로 실행해야 기존 동네로_크롤링 태스크 갱신)
- `telegram_bot.py` — 크롤링 미완료 라벨 07:00 → 08:30
- `GUIDE.md` / `CLAUDE.md` — 시간 표기 전체 08:30으로 갱신
- ⚠️ Windows 작업 스케줄러 `동네로_크롤링` — RunLevel Highest로 생성된 태스크라 관리자 권한 필요
  → setup_scheduler.ps1을 **관리자 권한으로 실행** 해야 적용됨 (현재 아직 07:00)

## [v0.95] 2026-05-19 — 케이스1 폴백 — 선택직종 공고 없으면 지역전체 발송

- `notify.py` — 표준직종 선택했으나 오늘 매칭 공고 0건 → 지역 전체 공고로 폴백, `#{job_type}=다양한 직종`
- 직종 미지정·직접입력만(케이스2)·폴백 모두 "다양한 직종" 통일
- 결과: 86명 전원 발송 (미발송 0명)

## [v0.94] 2026-05-19 — notify.py 발송 로직 고도화

- 소프트삭제 제외: `deleted_at IS NULL` 조건 추가 (휴지통 삭제자 발송 제외)
- 케이스1: 표준직종+직접입력 혼재 시 표준직종만 기준으로 매칭
- 케이스2: 직접입력만 있어 매핑 안 되는 경우 해당 지역 전체 공고 발송
- `get_mapped_tags()` 함수 추가 — 11개 표준직종 키 기준 분류
